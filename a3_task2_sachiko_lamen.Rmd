...
sw_split_name <- starwars %>% # Make new dataset 
  separate(col = name, # Separate the names column
           into = c("name_1","name_2"), # Into two columns 'name_1' & 'name_2'
           sep = "-") # separate the column at the first `-`

# Row 49 name = Ki-Adi-Mundi turns into name_1: Ki, name_2: Adi
```
...
sw_skin_sep <- starwars %>% # make new dataset
  separate(col = skin_color, # separate the `skin_color` column
           into = c("skin_1", "skin_2"), # into two columns
           sep = ",", # at the first `,`
           extra = "merge", # if additional `,` then merge all following into second column
           remove = FALSE)
```
...
mpg_model_class <- mpg %>% # make new dataset
  unite(col = model_class, # with column that
        c(model, class), #combines `model` and `class` columns into one
        sep = "-", # separate the united variables by a `-`
...
some_dates <- c(11061972, 02142007,01301843) # make dataframe

some_dates_converted <- mdy(some_dates) # convert to `Date` class
...
gasol_hookshots <- lakers %>% # make new subset
  filter(player == "Pau Gasol", type == "hook") %>% # only pull results for Pau Gasol and hook shots
  mutate(date = ymd(date)) %>% # change date value from numeric to date ymd format
  mutate(month = month(date)) %>% # make new column parsed from the date column
  count(month) # find number of results per month
...
# did not want to scroll to see what different storm types there were so I used the folowing code. Is there a better way to do this?
storm_type <- storms %>%
  group_by(status) %>%
  count(status)

# make storms_danger with new status column
storms_danger <- storms %>% # make new dataset 
  mutate(danger = case_when(status == "hurricane" ~ "high",
                            status %in% c("tropical depression", "tropical storm") ~ "moderate")) # create new column `danger` with `high` if `hurricane` and `moderate` if `tropical depression` or `tropical storm`
